...
training_df["DayofMonth"] = training_df["DayOfMonth"].astype(int)

test_df["DayOfMonth"] = test_df.Date.apply(lambda x: x.split("-")[2])
test_df["DayOfMonth"] = test_df["DayOfMonth"].astype(int)
...
with open("models/xgboost_regression_log2.pkl", "wb") as fid:
    pickle.dump(regressor, fid)

print("Model saved to models/xgboost_regression_log2.pkl")
########### TRAINING COMPLETED ##########

# Uncomment this block when not training
# with open("models/xgboost_regression_log2.pkl", "rb") as fid:
