...
def get_mean_dataframe(dft, dfs, column='Sales'):
    ...
    for store in stores:
        ...
        for day in days:
            mean_sales_days[day].append(
                serie[serie['DayOfWeek'] == day][column].mean())
        for month in months:
            mean_sales_months[month].append(
                serie[serie['Month'] == month][column].mean())
        for holiday in state_holidays:
            mean_sales_holiday[holiday].append(
                serie[serie['StateHoliday'] == holiday][column].mean())
    ...
    mean_sales_days = rename_dictionary(
        mean_sales_days, 'MeanDayOfWeek' + column)
    print(mean_sales_days)
    mean_sales_days['Store'] = stores
    df_days = pd.DataFrame(mean_sales_days)
    mean_sales_months = rename_dictionary(
        mean_sales_months, 'MeanMonth' + column)
    mean_sales_months['Store'] = stores
    df_months = pd.DataFrame(mean_sales_months)

    mean_sales_holiday = rename_dictionary(
        mean_sales_holiday, 'MeanHoliday' + column)
    mean_sales_holiday['Store'] = stores
    df_holidays = pd.DataFrame(mean_sales_holiday)

    # and normalize
    min_max_scaler = preprocessing.MinMaxScaler()
    df['Mean' + column] = min_max_scaler.fit_transform(df['Mean' + column].values.reshape([-1, 1]))
    df['Mean' + column + 'Promo'] = min_max_scaler.fit_transform(
        df['Mean' + column + 'Promo'].values.reshape([-1, 1]))
    df['Mean' + column + 'NotPromo'] = min_max_scaler.fit_transform(
        df['Mean' + column + 'NotPromo'].values.reshape([-1, 1]))

    for day in days:
        df_days['MeanDayOfWeek' + column + str(day)] = min_max_scaler.fit_transform(
            df_days['MeanDayOfWeek' + column + str(day)].values.reshape(-1, 1))

    for month in months:
        df_months['MeanMonth' + column + str(month)] = min_max_scaler.fit_transform(
            df_months['MeanMonth' + column + str(month)].values.reshape(-1, 1))

    for holiday in state_holidays:
        df_holidays['MeanHoliday' + column + str(holiday)] = min_max_scaler.fit_transform(
            df_holidays['MeanHoliday' + column + str(holiday)].values.reshape(-1, 1))
...
def load_data_file(filename, dtypes, parsedate=True):
    ...
    """
    def date_parse(x): return pd.datetime.strptime(x, '%Y-%m-%d')
    if parsedate:
        return pd.read_csv(
            filename,
            sep=',',
...
# Load data, parse data, clean unwanted columns, replace nan values,
# create column
print('Loading data ...')
data_dir = '../../data/'
...
data_train['WeekOfMonth'] = data_train['Date'].apply(
    lambda x: int(week_of_month(x)))
data_test['Year'] = data_test['Date'].apply(lambda x: str(x)[:4])
data_test['Month'] = data_test['Date'].apply(lambda x: int(str(x)[5:7]))
data_test['WeekOfMonth'] = data_test['Date'].apply(
    lambda x: int(week_of_month(x)))
...
data_train = pd.concat([data_train, pd.get_dummies(
    data_train['StateHoliday'], dummy_na=True, prefix='StateHoliday')], axis=1)
data_test = pd.concat([data_test, pd.get_dummies(
    data_test['StateHoliday'], dummy_na=True, prefix='StateHoliday')], axis=1)
data_train = pd.concat([data_train, pd.get_dummies(
    data_train['WeekOfMonth'], dummy_na=True, prefix='WeekOfMonth')], axis=1)
data_test = pd.concat([data_test, pd.get_dummies(
    data_test['WeekOfMonth'], dummy_na=True, prefix='WeekOfMonth')], axis=1)
data_train = pd.concat([data_train, pd.get_dummies(
    data_train['Year'], dummy_na=True, prefix='Year')], axis=1)
data_test = pd.concat([data_test, pd.get_dummies(
    data_test['Year'], dummy_na=True, prefix='Year')], axis=1)
data_train = pd.concat([data_train, pd.get_dummies(
    data_train['Month'], dummy_na=True, prefix='Month')], axis=1)
data_test = pd.concat([data_test, pd.get_dummies(
    data_test['Month'], dummy_na=True, prefix='Month')], axis=1)
data_train = pd.concat([data_train, pd.get_dummies(
    data_train['DayOfWeek'], dummy_na=True, prefix='DayOfWeek')], axis=1)
data_test = pd.concat([data_test, pd.get_dummies(
    data_test['DayOfWeek'], dummy_na=True, prefix='DayOfWeek')], axis=1)


# in datatest column some columns is missing so we need to add it with
# default 0
for column in data_train.columns.values.tolist():
    if column not in data_test.columns.values.tolist():
        data_test[column] = 0

data_store = pd.concat([data_store, pd.get_dummies(
    data_store['Assortment'], dummy_na=True, prefix='Assortment')], axis=1)
data_store = pd.concat([data_store, pd.get_dummies(
    data_store['StoreType'], dummy_na=True, prefix='StoreType')], axis=1)

# this is simple converting to integer
data_train['StateHoliday'] = replace_values(
    data_train,
    'StateHoliday',
    StateHoliday).astype(
        np.int8)
data_test['StateHoliday'] = replace_values(
    data_test,
    'StateHoliday',
    StateHoliday).astype(
        np.int8)
data_store['Assortment'] = replace_values(
    data_store,
    'Assortment',
    Assortment).astype(
        np.int8)
data_store['StoreType'] = replace_values(
    data_store,
    'StoreType',
    StoreType).astype(
        np.int8)
...
data_store['CompetitionDistance'] = data_store['CompetitionDistance'].fillna(
    meanCompetitionDistance)
data_store['CompetitionDistance'] = data_store['CompetitionDistance'].astype(
    np.float32)
...
min_max_scaler = preprocessing.MinMaxScaler()
data_store['CompetitionDistance'] = min_max_scaler.fit_transform(
    data_store['CompetitionDistance'].values.reshape([-1, 1]))
